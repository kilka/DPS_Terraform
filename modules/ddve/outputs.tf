...
output "iam_role_name" {
  ...
  value       = local.create_iam_role ? aws_iam_role.ddve[0].name : data.aws_iam_instance_profile.existing[0].role_name
}

output "iam_role_arn" {
  description = "ARN of the IAM role attached to the DDVE instance"
  value       = local.create_iam_role ? aws_iam_role.ddve[0].arn : data.aws_iam_instance_profile.existing[0].role_arn
}

output "iam_instance_profile_name" {
  description = "Name of the IAM instance profile"
  value       = local.create_iam_role ? aws_iam_instance_profile.ddve[0].name : var.iam_instance_profile_name
}
...
output "ami_id" {
  description = "ID of the AMI used for the DDVE instance"
  value       = data.aws_ami.ddve.id
}
